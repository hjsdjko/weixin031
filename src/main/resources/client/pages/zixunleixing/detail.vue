	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('zixunleixing', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zixunleixing');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','zixunleixing');
			...
			async init(){
				...
				let res = await this.$api.info('zixunleixing', this.id);
				this.detail = res.data;
			},
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discusszixunleixing/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
