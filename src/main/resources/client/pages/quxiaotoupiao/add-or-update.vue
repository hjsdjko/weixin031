	...
	<view :style='{"width":"100%","padding":"0 0 0 0","position":"relative","background":"#ffffff","height":"100%"}'>
		<form :style='{"width":"100%","padding":"24rpx 24rpx 24rpx 24rpx","background":"#ffffff","display":"block","height":"auto"}' class="app-update-pv">
			...
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0 0 2rpx 0","background":"#ffffff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				...
				<input :style='{"border":"0px solid #eeeeee","padding":"0px 24rpx 0px 24rpx","margin":"0 0 0 0","color":"#666666","borderRadius":"8rpx","flex":"1","background":"#ffffff00","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' :disabled="ro.toupiaoshu" v-model="ruleForm.toupiaoshu" placeholder="投票数"></input>
			</view>
			<view :style='{"padding":"4rpx 0 4rpx 0","margin":"0 0 24rpx 0","borderColor":"#eeeeee","alignItems":"center","borderWidth":"0 0 2rpx 0","background":"#ffffff","display":"flex","width":"100%","borderStyle":"solid","height":"auto"}' class="">
				<view :style='{"width":"160rpx","padding":"0 20rpx 0 0","lineHeight":"80rpx","fontSize":"28rpx","color":"#333333","textAlign":"right"}' class="title">取消投票时间</view>
				<input :style='{"border":"0px solid #eeeeee","padding":"0px 24rpx 0px 24rpx","margin":"0 0 0 0","color":"#666666","borderRadius":"8rpx","flex":"1","background":"#ffffff00","fontSize":"28rpx","lineHeight":"80rpx","height":"80rpx"}' v-model="ruleForm.quxiaotoupiaoshijian" placeholder="取消投票时间" @tap="toggleTab('quxiaotoupiaoshijian')"></input>
			</view>
		...
		<w-picker mode="dateTime" step="1" :current="false" :hasSecond="false" @confirm="quxiaotoupiaoshijianConfirm" ref="quxiaotoupiaoshijian" themeColor="#333333"></w-picker>
	</view>
</view>
</template>
...
<script>
	...
	export default {
		...
		async onLoad(options) {
            this.ruleForm.quxiaotoupiaoshijian = this.$utils.getCurDateTime();
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`quxiaotoupiao`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					...
					if(o=='quxiaotoupiaoshijian'){
					this.ruleForm.quxiaotoupiaoshijian = obj[o];
					this.ro.quxiaotoupiaoshijian = true;
					continue;
					}
					if(o=='tupian'){
		...
		methods: {
			...
			// 日长控件选择日期时间
			quxiaotoupiaoshijianConfirm(val) {
				console.log(val)
				this.ruleForm.quxiaotoupiaoshijian = val.result;
				this.$forceUpdate();
			},
...
//跨表计算判断
				...
				//obj.toupiaoshu = obj.toupiaoshu - this.ruleForm.toupiaoshu
                if((parseFloat(obj.toupiaoshu) - parseFloat(this.ruleForm.toupiaoshu))<0){
					this.$utils.msg("投票数不足");
					return
				}
				if(this.ruleForm.toupiaoshu&&(!this.$validate.isIntNumer(this.ruleForm.toupiaoshu))){
					this.$utils.msg(`投票数应输入整数`);
					return
				}
				//更新跨表属性
			       var crossuserid;
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`quxiaotoupiao`, params);
                ...
                //跨表计算
                        ...
                        var table = uni.getStorageSync('crossTable');
                        obj.toupiaoshu = parseFloat(obj.toupiaoshu) - parseFloat(this.ruleForm.toupiaoshu)
                        await this.$api.update(`${table}`, obj);
						if(this.ruleForm.id){
							await this.$api.update(`quxiaotoupiao`, this.ruleForm);
						}else{
							await this.$api.add(`quxiaotoupiao`, this.ruleForm);
						}
