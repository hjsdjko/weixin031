	...
	export default {
		...
		async onShow(options) {
            ...
            if(crossCleanType) {
                ...
                res = await this.$api.info('zixun', this.id);
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','zixun');
			...
			onAcrossTap(tableName,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				uni.setStorageSync('crossTable','zixun');
			...
			async init(){
				...
				let res = await this.$api.info('zixun', this.id);
				this.detail = res.data;
				//修改富文本的图片样式
			...
			async onCommentTap() {
				...
				this.$utils.jump(`../discusszixun/add-or-update?refid=${this.id}`)
			},
			onSHTap() {
				if(!this.user){
